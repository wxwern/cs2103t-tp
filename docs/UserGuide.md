...
`DEADLINE` | A valid deadline should be in the form *DD-MM-YYYY*.<br><br>The day `DD` and month `MM` can be either single or double digits. | `09-02-2022`<br>`9-2-2022`<br>`19-11-2022`
`APPLICATION STAGE` | A valid application stage can only accept one of the three values: *resume, online assessment, interview*. | `resume`<br>`online assessment`<br>`interview`
`STATUS` | A valid status can only accept one of the four values: *pending, offered, accepted, turned down*. | `pending`<br>`offered`<br>`accepted`<br>`turned down`
